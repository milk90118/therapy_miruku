    ...
    *,
    ...
    try:
        resp_kwargs: dict[str, Any] = dict(
            ...
            max_output_tokens=170,  # max_output_tokens 是 Responses API 的輸出上限:contentReference[oaicite:4]{index=4}
            # ✅ Step 4: 心理內容預設不存（可自行改 True）
            store=False,
        )

        # previous_response_id 可串接狀態；且 instructions 不會自動繼承，所以你每次重送是正確的:contentReference[oaicite:5]{index=5}
